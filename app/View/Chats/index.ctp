...
$(document).ready(function(){
    ...
    var filesExt = ['jpg', 'gif', 'png','docx','doc','pdf','xml','ppt','xls','jpeg','txt','xlsx']; // массив расширений
    ...
    if(filesExt.join().search(parts[parts.length - 1]) != -1){
        ...
        var file_data = $('#chat_file_input').prop('files')[0];
        ...
        if(file_data) {
            $.ajax({
                ...
                success: function(data){
                    ...
                    $('.chat_message').attr('disabled', 'disabled');
    ...
    //"<img src='/img/test/"+php_script_response+"' />"
        ...
        $('.chat_send_btn').on('click', function(e){
            ...
            var filesExt = ['jpg', 'gif', 'png','docx','doc','pdf','xml','ppt','xls','jpeg','txt','xlsx'];
           
            if(filesExt.join().search(path) != -1){
                msg =  '<a href="/files/messages/'+text+'" download><img src="/img/icon.png"/></a>'   
            }else {
                msg = text;
            }
            ...
            if(text.trim().length>0){
                ...
                var message  = {"on":"message","data":{"from": uid, "to":chatId, "text":msg, "time":time}};
