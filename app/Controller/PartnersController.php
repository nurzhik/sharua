...
class PartnersController extends AppController{
	public $uses = array('Category', 'Partner');
	...
	public function index(){
		...
		$order = array('Partner.sort_order DESC');
		$paginator = array();
		...
		$paginator['count'] = $this->Partner->find('count') - 1;
	...
	public function view($id){
		...
		$other_partner = $this->Partner->find('all', array(
			'conditions' => array(array('Partner.id !=' => $id)),
			'limit' => 6,
		));
		$title_for_layout = $data['Partner']['title'];
		$meta['keywords'] = $data['Partner']['keywords'];
		$meta['description'] = $data['Partner']['description'];

		$this->set(compact('data', 'title_for_layout', 'other_partner', 'meta'));
	}
